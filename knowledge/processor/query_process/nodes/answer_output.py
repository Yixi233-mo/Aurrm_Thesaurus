...
from knowledge.tools.task_utils import set_task_result, set_task_sources, get_done_task_list, get_running_task_list
...
class AnswerOutputNode(BaseNode):
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        # 3. 持久化来源（非流式模式通过任务结果返回；流式模式由 SSE FINAL 推送）
        if not is_stream:
            set_task_sources(session_id, self._last_sources)
        state["sources"] = self._last_sources
        ...
        # 5. 流式模式发送结束事件
        if is_stream:
            self.logger.info(f"FINAL event: answer_len={len(state.get('answer',''))}, sources_count={len(self._last_sources)}")
            for s in self._last_sources:
                self.logger.info(f"  source: {s}")
            push_sse_event(session_id, SSEEvent.FINAL,
                           {"answer": state.get("answer", ""), "sources": self._last_sources})
        return state
    ...
    def _generate_answer(self, state, prompt):
        ...
        if state.get("is_stream"):
            state["answer"] = self._stream_generate(llm_client, prompt, session_id)
        else:
    ...
    def _format_reranked_docs(self, reranked_docs: List[Dict], char_budget: int) -> Tuple[str, int]:
        ...
        for idx, doc in enumerate(reranked_docs, 1):
            ...
            source = {"idx": idx}
            for key in ["source", "chunk_id", "title", "url", "score"]:
                value = doc.get(key)
    ...
    def _stream_generate(self, llm_client, prompt, task_id):
        ...
        accumulated_answer = ""
        try:
            # 告知前端：开始生成答案（填补 rerank 完成到首 token 之间的空白）
            push_sse_event(task_id, SSEEvent.PROGRESS, {
                "done_list": get_done_task_list(task_id),
                "running_list": get_running_task_list(task_id) + ["生成答案"],
            })
            for chunk in llm_client.stream(prompt):
                delta_text = getattr(chunk, "content", "") or ""
                if delta_text:
                    # 清洗每个 delta chunk 中的 advisor 文本，避免污染客户端
                    cleaned_delta = re.sub(r'\[Advisor consultation.*?\]', '', delta_text, flags=re.DOTALL)
                    cleaned_delta = re.sub(r'\[Advisor review\]', '', cleaned_delta)
                    cleaned_delta = re.sub(r'\[End of advisor consultation.*?\]', '', cleaned_delta, flags=re.DOTALL)
                    if cleaned_delta:
                        accumulated_answer += cleaned_delta
                        push_sse_event(task_id, "delta", {"delta": cleaned_delta})
        except Exception as e:
