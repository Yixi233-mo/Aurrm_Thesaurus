...
from knowledge.tools.task_utils import get_task_status, TASK_STATUS_COMPLETED
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                # 15s 超时：足够覆盖节点空闲间隙，又能及时检测断开
                msg = await loop.run_in_executor(None, stream_queue.get, True, 15.0)
            except queue.Empty:
                # 7. 15s 内无数据，发送 SSE 注释心跳（浏览器重置闲置计时器，不触发 onmessage）
                yield ":heartbeat\n\n"
                # 心跳后再次检查断开状态
                if await request.is_disconnected():
                    break
        ...
        # 仅在任务完成后清理队列；若任务仍在运行（客户端中途断开），保留队列以便重连
        if get_task_status(task_id) == TASK_STATUS_COMPLETED:
